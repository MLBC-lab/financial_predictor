...
RESULT_OFILE = '../../output/cross_validate_result_scale_no_pca_no_mva_t3_1.pckl'
# INLIER = (0.10, 0.90)
...
def clean_feats_targets(feat, targ):
    ...
    # low, high = tuple(np.quantile(targ, INLIER))
    # feat = feat[np.logical_and(low <= targ, targ <= high)]
    # targ = targ[np.logical_and(low <= targ, targ <= high)]
...
def process(data, trts, tg):
    ...
    print(f'Dur: {time.time() - t0:.0f} s, '
          ...
          f'TrMSE: {np.round(result["tr_mse"], 4)}, '
          f'TsMSE: {np.round(result["ts_mse"], 4)}\n'
          f'{result["estimator"]}')
    return result
...
def main(argv):
    ...
    #                        [(data, trts, tg) for tg in list(trts.keys())[:]])
    # Only for 'mva_t3_1'
    result = process(data, trts, 'mva_t3_1')
    res.append(result)
    # Save the results
